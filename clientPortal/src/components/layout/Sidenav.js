...
const Sidenav = ({ color, userinfo })=>{
  ...
  const DisplaySideNav = (routes)=>{
    return routes.map((item,key)=>{
      return  <Menu theme="light" mode="inline">
        <Menu.Item className={item.to==="header"? "menu-item-header" : ""} key={key+1}>
          {
            item.to==="header"?
              item.label
            :
            <NavLink to={item.to} hidden={item.to==="/PaymentStatus"}>
              <span
                className="icon"
                style={{
                  background: page === item.to ? color : "",
                }}
              >
                {item.icon}
              </span>
